...
feature 'user receives suggestions for movies' do
  ...
  let!(:minion1) { FactoryGirl.create(:suggestion, genre: family, topping: cheese) }
  let!(:minion2) { FactoryGirl.create(:suggestion, genre: adventure, topping: cheese) }
  let!(:minion3) { FactoryGirl.create(:suggestion, genre: animation, topping: cheese) }
